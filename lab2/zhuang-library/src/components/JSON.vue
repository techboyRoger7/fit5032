...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <!-- TODO: CODE TO RENDER LIST OF OPENING HOURS HERE -->
       <li v-for="(hours, days) in bookstores.openingHours" :key="days">
          {{ days }} Open: {{ hours.open }} Close: {{ hours.close }}
       </li>
    ...
    <section class="lab-section">
      ...
      <p :style="{color: nameColor, fontSize: nameSize + 'px'}">{{ authorName.name }}</p>
    </section>
  </div>
</template>
...
const allFamousWorks = computed(() => {
  ...
  return authors.flatMap((a) => a.famousWorks.map((t) => t.title));
})

const companyName = computed(() => bookstores.name);
const nameSize = ref(18);
const nameColor = ref('blue');

const authorName = computed( () => authors.find((a) => a.name == "George Orwell"));
</script>
